...
def main():
    ...
    # optionally resume from a checkpoint
    if args.resume:
        model.load_state_dict(torch.load(os.path.join(base_path, '2024-01-04_19H/model.pth')))
...
def train(train_loader, model, criterion, optimizer, epoch, dir, mask_model = None):
    ...
    for i, (inputs, target) in enumerate(train_loader):
        ...
        if args.mask == True:
            output, l1, l2, l3, hmaps, _, f = mask_model(inputs, target)
        ...
        # save_cam(inputs[0], hmaps_f, i*len(inputs), conf=True)
        # for j in range(len(inputs)):
        #     cnt += 1
        #     d_score += get_hscore(hmaps_t[j], hmaps_f[j])
