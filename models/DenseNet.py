...
class DenseNet(nn.Module):
    ...
    def __init__(self, growth_rate=32, block_config=(6, 12, 24, 16),
                 num_init_features=64, bn_size=4, drop_rate=0.0, num_classes=5, memory_efficient=False, last_activation=None, plus = True):
        ...
        for i, num_layers in enumerate(block_config):
            ...
            if i != len(block_config) - 1:
                ...
                if plus == True:
                    ...
                    # num_features = num_features + num_layers * growth_rate
                    # self.last_norm = nn.BatchNorm2d(num_features, momentum=0.01)
                    # num_layers = 1
                    num_features = 1
                    for j in range(num_classes):
                        self.last_blocks.append(_DenseBlock(
                        num_layers=num_layers,
                        num_input_features=num_features,
                        bn_size=bn_size,
                        growth_rate=growth_rate,
                    ...
                    # self.features.add_module('last_blocks', self.last_blocks)
                    num_features = num_features + num_layers * growth_rate
                    self.last_norm = nn.BatchNorm2d(num_features * num_classes, momentum=0.01)
                    self.classifier = nn.Linear(num_features * num_classes, num_classes)
                else:
                    block = _DenseBlock(
                    num_layers=num_layers,
                    num_input_features=num_features,
    ...
    def forward(self, x, parallel_last):
        features = self.features(x)
        if parallel_last == False:
            features = self.last_block(features)
            features = self.last_norm(features)
        else:
            # print("done")
            concat = self.last_blocks[0](features)
            for i in range(1, self.num_classes):
                concat = torch.cat([concat, self.last_blocks[i](features)], dim=1) # BS x class_num * 512 x 4 x 4
            features = self.last_norm(concat)
...
def _densenet(arch, growth_rate, block_config, num_init_features, pretrained, progress, plus,
              **kwargs):
    model = DenseNet(growth_rate, block_config, num_init_features, plus = plus, **kwargs)
    if pretrained:
        _load_state_dict(model, model_urls[arch], progress)
    return model


def DenseNet121(pretrained=False, progress=True, activations='relu', plus = False , **kwargs):
    ...
    return _densenet('densenet121', 32, (6, 12, 24, 16), 64, pretrained, progress, plus = plus, 
                     **kwargs)
...
def DenseNet201(pretrained=True, progress=True, activations='relu', **kwargs):
